...
zap_translated AS (
    SELECT
        ...
        (CASE
            WHEN COALESCE(dcp_certifiedreferred, dcp_dcptargetcertificationdate) IS NULL THEN NULL
...
--descriptor fields
    ...
    COALESCE(
        ...
        END),
        (CASE
            WHEN NULLIF(dcp_mihdushighernumber + dcp_noofvoluntaryaffordabledus, 0) IS NOT NULL
                THEN 'dcp_mihdushighernumber + dcp_noofvoluntaryaffordabledus'
        END),
        (CASE
            WHEN NULLIF(dcp_mihduslowernumber + dcp_noofvoluntaryaffordabledus, 0) IS NOT NULL
