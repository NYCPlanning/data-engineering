...
- [Bug 017](./017-saf-abcegnpx-stale-prod-source-snapshot.md): `saf_abcegnpx_generic`/`saf_abcegnpx_roadbed`'s small row-count-only diffs (7 and 3 rows) traced to source, not GR-confirmed - retracts the earlier "unimplemented SAF-replicant scope" guess. 6 of 7 rows fit the same stale-prod-source-snapshot pattern as `CSCL-SAF-01`/`Bug 010`/`Bug 011`; the 7th (`9017010`) turned out NOT to be staleness at all - a single source row with a blank `ALT_SEGDATA_TYPE` that our filter spec-correctly excludes, unrelated to the other 6
