...
import pytest
from aggregate.config import acs_current_year_suffix
...
def test_acs_datasets_no_semantic_duplicates(resource_name):
    ...
    """
    if resource_name == "acs_current_year_band":
        pytest.xfail(
            "The 2020-2024 ACS PUMS export has both P16t64_* and P16t64y_* "
            "columns for the same underlying variable. Needs a fix upstream "
            "in the source Excel, not in this repo."
        )
    try:
        df = load(resource_name)
    except FileNotFoundError:
...
# Citywide sanity ranges for the current ACS year band, source: product owner.
# Column prefixes follow the ACS PUMS export convention (e.g. "Pop1_24E" for the
# 2024 vintage); the year suffix is derived from the recipe's ACS_CURRENT_YEAR_BAND
# (see aggregate/config.py) so this doesn't need a manual update each vintage bump.
# (column prefix, label, exclusive lower bound, exclusive upper bound)
CITYWIDE_SANITY_RANGES = [
    ("Pop1", "Total Population", 7_000_000, 10_000_000),
    ("MdAge", "Median Age", 30, 50),
    ("MdHInc", "Median Household Income", 40_000, 120_000),
    ("MdGR", "Median Gross Rent", 1_000, 2_500),
    ("MdVl", "Median Home Value", 400_000, 900_000),
]


@pytest.mark.parametrize(
    "prefix, label, lower, upper",
    CITYWIDE_SANITY_RANGES,
    ids=[label for _, label, _, _ in CITYWIDE_SANITY_RANGES],
)
def test_citywide_acs_value_in_sane_range(prefix, label, lower, upper):
    """
    Guard against a bad ACS pull (e.g. wrong units, mis-mapped column, decimal
    error) by checking that citywide headline values fall within a plausible
    range. These bounds are wide on purpose - they aren't meant to catch a
    normal year-over-year change, only a value that's obviously wrong.
    """
    column = f"{prefix}_{acs_current_year_suffix}E"
    df = load("acs_current_year_band")
    row = df.set_index("Geog").loc["NYC"]

    value = row[column]
    assert lower < value < upper, (
        f"Citywide {label} ({column}) is {value}, expected to be between "
        f"{lower} and {upper}"
    )
