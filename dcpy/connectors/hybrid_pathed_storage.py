...
class HybridPathedStorage:
    ...
    class Factory:
        ...
        def s3(
            ...
            s3_file_metadata: Optional[dict] = None,
        ) -> "HybridPathedStorage":
...
class PathedStorageConnector:  # (StorageConnector):
    ...
    def push(self, key: str, **kwargs) -> dict:
        ...
        acl = kwargs.get("acl")
        if acl == "public-read" and not self.storage.storage_type != StorageType.S3:
            raise ValueError("Public ACL is only supported for S3 storage")
        ...
        if self.storage.storage_type == StorageType.S3:
            upload_storage = HybridPathedStorage.Factory.s3(
                root_folder=self._base_storage_kwargs.get("root_folder") or "",
                s3_bucket=self._base_storage_kwargs["s3_bucket"],
            ...
            upload_storage = self.storage

        dest_path = upload_storage.root_path / key

        if source_path.is_dir():
            if dest_path.exists():
                dest_path.rmtree()
                dest_path.copytree(source_path)
        else:
        ...
        if upload_storage.storage_type == StorageType.AZURE and metadata:
            blob = dest_path.client.service_client.get_blob_client(
                dest_path.container, dest_path.blob
            )
    ...
    def get_metadata(self, key: str):
        ...
        if isinstance(client, AzureBlobClient):
            return client._get_metadata(self.storage.root_path / key).get(
                "metadata", {}
            )  # type: ignore
