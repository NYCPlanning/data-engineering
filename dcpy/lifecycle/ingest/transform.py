    ...
    def python_script(
        ...
        **kwargs,
        ...
        if sig.return_annotation != inspect.Signature.empty:
            ...
            if (
                "DataFrame" not in return_type_str
                and "GeoDataFrame" not in return_type_str
            ):
                raise ValueError(
        ...
        if is_geodataframe and not isinstance(result_df, gpd.GeoDataFrame):
            ...
            if original_geom_col and original_geom_col in result_df.columns:
                result_df = gpd.GeoDataFrame(
                    result_df, geometry=original_geom_col, crs=original_crs
                )
                logger.info(
                ...
                result_df = gpd.GeoDataFrame(
                    result_df, geometry="geometry", crs=original_crs
                )
                logger.info(
