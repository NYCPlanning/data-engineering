...
def xml_to_csv(xml_path: Path, output_csv: Path) -> None:
    ...
    for field in file_fields.findall('.//ns:FileField', ns) or file_fields.findall('.//FileField'):
    ...
    # Write CSV
    fieldnames = ['field_number', 'field_name', 'field_label', 'field_length', 
                  'start_index', 'end_index', 'justify_and_fill', 'blank_if_none']
