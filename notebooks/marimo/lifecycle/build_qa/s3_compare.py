...
def _(bucket_input, path_a_input, path_b_input):
    def _fetch(prefix: str) -> pd.DataFrame:
        ...
        if not objs:
            return pd.DataFrame(
                columns=["filename", "size_bytes", "last_modified"]
            )
        return pd.DataFrame(
            [
                {
...
def _(df_a, df_b, path_a_input, path_b_input):
    ...
    _merged["size_diff_pct"] = (
        (_merged["size_diff_bytes"] / _merged["size_a"] * 100).round(1)
    )
...
@app.cell
def _(comparison):
    _df = comparison[~comparison["in_b"]][["filename", "size_a"]].reset_index(
        drop=True
    )
...
@app.cell
def _(comparison):
    _df = comparison[~comparison["in_a"]][["filename", "size_b"]].reset_index(
        drop=True
    )
    mo.ui.table(_df, selection=None) if len(_df) else mo.md(
...
def _(comparison):
    _df = (
        ...
        .sort_values(
            "size_diff_pct", key=abs, ascending=False, na_position="last"
        )
...
def _(table_selector, tbl_a, tbl_b):
    ...
    **Columns:** {_col_status}
    """),
            mo.ui.tabs(
                {
                    f"A — {table_selector.value}": mo.ui.table(
                        tbl_a, selection=None
                    ),
                    f"B — {table_selector.value}": mo.ui.table(
                        tbl_b, selection=None
                    ),
                }
            ),
        ]
