...
WITH exact_matches AS (
    SELECT
        ...
        r.source_feature_type AS joined_segment_feature_type,
        'exact' AS match_type,
        0 AS distance
    FROM {{ ref('int__segments') }} AS s
    INNER JOIN {{ ref('int__segments') }} AS r ON ST_EQUALS(r.geom, s.geom)
    WHERE s.source_feature_type NOT IN ('centerline', 'nonstreetfeatures')
),
fuzzy_matches AS (
    SELECT
        s.segmentid,
    ...
    WHERE s.source_feature_type NOT IN ('centerline', 'nonstreetfeatures') AND NOT EXISTS (
