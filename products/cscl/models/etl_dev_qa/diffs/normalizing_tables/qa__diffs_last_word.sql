...
-- Compressed diff view showing one row per _last_word_key with changes in jsonb
-- Includes status: modified, only_in_legacy, only_in_build

WITH base_diffs AS (
  {{ generate_diff_summary(
      old_relation=ref('qa_int__prod_last_word'),
      new_relation=ref('last_word'),
      primary_key='_last_word_key',
      output_file_id='last_word',
      build_table_name='last_word',
      production_table_name='qa_int__prod_last_word'
  ) }}
),
categorized AS (
    SELECT
...
SELECT
    _last_word_key AS comparison_id,
    status,
    changes,
    output_file_id,
