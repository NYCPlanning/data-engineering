...
def _stringify(df: pd.DataFrame, cols: list[str]) -> pd.DataFrame:
    ...
    Two normalizations, both aimed at not fooling ourselves into reporting a
...
def _row_level_diff(
    ...
    if len(common) > 0 and compare_cols:
        dev_common = _stringify(dev_indexed.loc[common], compare_cols)
        prod_common = _stringify(prod_indexed.loc[common], compare_cols)
