...
def categorization_summarization_statistics(df, category):
    ...
    print(
        "% projects joined to CPDB geometries and categorized as {}: {}".format(
            category, round((m / n) * 100, 2)
        )
    )
    print(
        "% total money captured by Checkbook NYC represented by joined projects categorized as {}: {}".format(
            category,
            round((sum(cat_geoms["check_amount"]) / sum(df["check_amount"])) * 100, 2),
        )
    )
    return
...
def geometries_summarization_statistics(df):
    ...
    print(
        "Percent of projects mapped to geometries: {}".format(round((m / n) * 100, 2))
    )
    print(
        "Amount of money mapped to geometries: {}".format(
            round(sum(geoms["check_amount"]), 2)
        )
    )
    print(
        "Percent of money mapped to geometries: {}".format(
            round((sum(geoms["check_amount"]) / sum(df["check_amount"])))
        )
    )
    return
...
def categorization_run_summary_statistics():
    final_categorization_file = pd.read_csv(
        _curr_file_path.parent.parent / "output" / "temp_historical_spend.csv"
    )
