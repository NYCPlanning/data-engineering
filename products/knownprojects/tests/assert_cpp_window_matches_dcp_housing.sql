...
-- Tagged explicitly: this test's only dependency is a source, and dbt's
-- indirect test selection only pulls in tests whose parent models are selected.
-- Without the tag it is never chosen by `tag:cpp`.
