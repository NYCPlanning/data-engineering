...
class TestProcessors:
    ...
    def test_python_script_not_callable(self):
        ...
        with pytest.raises(
            ValueError, match="'NOT_CALLABLE' in module .* is not callable"
        ):
            self.proc.python_script(
                self.basic_df,
                module="dcpy.test.lifecycle.ingest.resources.test_scripts",
    ...
    def test_python_script_wrong_return_type(self):
        ...
        with pytest.raises(
            ValueError,
            match="return type hint is '<class 'list'>'.*expected pd.DataFrame",
        ):
            self.proc.python_script(
                self.basic_df,
                module="dcpy.test.lifecycle.ingest.resources.test_scripts",
    ...
    def test_python_script_with_type_hint(self):
        ...
        assert all(result.df["type_hinted"])

    def test_python_script_wrong_type_hint(self):
        """Test error when function has wrong return type hint"""
        with pytest.raises(
            ValueError,
            match="return type hint is '<class 'str'>'.*expected pd.DataFrame",
        ):
            self.proc.python_script(
                self.basic_df,
                module="dcpy.test.lifecycle.ingest.resources.test_scripts",
    ...
    def test_python_script_wrong_return_at_runtime(self):
        ...
        with pytest.raises(
            ValueError, match="must return a pd.DataFrame or gpd.GeoDataFrame"
        ):
            self.proc.python_script(
                self.basic_df,
                module="dcpy.test.lifecycle.ingest.resources.test_scripts",
