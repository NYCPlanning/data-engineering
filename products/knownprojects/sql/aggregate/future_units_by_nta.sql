...
nta_details AS (
    SELECT * FROM {{ source('recipe_sources', 'dcp_nta2020') }}
),
...
future_units_with_nta AS (
    SELECT
        ...
        units_net,
        units_net_in_nta,
        future_phased_units_total_in_nta,
        future_units_without_phasing_in_nta,
        completed_units_in_nta,
...
future_units_by_nta AS (
    SELECT
        ...
        sum(future_phased_units_total_in_nta) AS future_phased_units_total,
        sum(future_units_without_phasing_in_nta) AS future_units_without_phasing,
        sum(completed_units_in_nta) AS completed_units,
