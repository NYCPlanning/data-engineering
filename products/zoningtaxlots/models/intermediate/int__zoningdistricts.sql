...
initial_rankings AS (
    SELECT
        ...
        ROW_NUMBER()
            OVER (PARTITION BY dtm_id ORDER BY segbblgeom DESC)
        AS lot_row_number,
...
lotzoneperorder_init AS (
    SELECT
        ...
        CASE
            WHEN
                ROW_NUMBER() OVER (PARTITION BY dtm_id ORDER BY segbblgeom DESC)
                = 1
                OR LAG(segbblgeom, 1, segbblgeom)
                    OVER (PARTITION BY dtm_id ORDER BY segbblgeom DESC)
                - segbblgeom
                > 0.01
                THEN 1
...
group_column_added AS (
    SELECT
        ...
        row_number,
        SUM(group_start)
            OVER (PARTITION BY dtm_id ORDER BY row_number)
        AS reorder_group
...
reorder_groups AS (
    SELECT
        dtm_id,
        reorder_group,
        -- starting point for re-ordering in "new_order" CTE down below
        MIN(row_number) - 1 AS order_start,
...
new_order AS (
    SELECT
        ...
        g.zonedist,
        ROW_NUMBER()
            OVER (PARTITION BY dtm_id, reorder_group ORDER BY priority ASC)
...
lotzoneperorder AS (
    ...
    LEFT JOIN
        new_order AS new
        ON a.dtm_id = new.dtm_id AND a.zonedist = new.zonedist
)
