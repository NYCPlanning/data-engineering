...
-- temporary workaround 2023-04: geosupport is returning latest city council districts, but we need a specific version
    ...
    SELECT
        ...
        (SELECT zipcode FROM doitt_zipcodeboundaries AS b WHERE st_intersects(b.wkb_geometry, a.geom) LIMIT 1
        ) AS zipcode,
        (SELECT bin::bigint::text FROM doitt_buildingfootprints AS b WHERE st_intersects(b.wkb_geometry, a.geom) LIMIT 1
        ) AS bin,
        (SELECT bbl::bigint::text FROM dcp_mappluto_wi AS b WHERE st_intersects(b.wkb_geometry, a.geom)) AS bbl,
        (SELECT upper(po_name) FROM doitt_zipcodeboundaries AS b WHERE st_intersects(b.wkb_geometry, a.geom) LIMIT 1
        ) AS city,
        ...
        (
            SELECT schooldist::text FROM dcp_school_districts AS b WHERE st_intersects(b.wkb_geometry, a.geom)
        ) AS schooldist,
