# db-gru-qaqc ![CI](https://github.com/NYCPlanning/db-gru-qaqc/actions/workflows/main.yml/badge.svg)

This repository contains automated QAQC checks for various GRU-maintained files. Checks are performed either by comparing files to each other or by comparing a file to the latest Geosupport release.

**To perform a check and view outputs:** see the gru page in the [Data Engineering QAQC App](https://de-qaqc.nycplanningdigital.com/?page=GRU+QAQC).
...
This check extracts street names from the DCM, and checks that these names can be normalized and matched with a geosupport code. To do so, the street name and borough from the DCM street centerline file are inputs to function 1N. Name - borough combinations that do not yeild a '00' return code are in the QAQC file.
