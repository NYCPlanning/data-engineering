...
S3_PATH = f"ingest_datasets/{DATASET}/20240815/{DATASET}.parquet"
...
def test_run(mock_request_get, create_buckets, create_temp_filesystem):
    ...
    run(dataset=DATASET, staging_dir=create_temp_filesystem)
...
def test_skip_archival(mock_request_get, create_buckets, create_temp_filesystem):
    run(dataset=DATASET, staging_dir=create_temp_filesystem, skip_archival=True)
