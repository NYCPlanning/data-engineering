...
from (
        ...
        select
            ...
            case
                when concat(a.source, a.record_id) in (select concat(source, record_id) from multi_geocoded_projects) and st_area(a.geometry) > 0
                    then
                        (st_area(st_intersection(a.geometry, a.subdist_geom)) / st_area(a.geometry))::decimal
                else
                    1
    ...
    aggregated_boundaries_subdist_4 as (
        select
            a.*,
            case
                when b.total_proportion is not null then (a.proportion_in_subdist / b.total_proportion)::decimal
                else 1
            end as proportion_in_subdist_1,
            case
                when b.total_proportion is not null then round(a.units_net * (a.proportion_in_subdist / b.total_proportion)::decimal)
                else a.units_net
            end as units_net_1
        from
