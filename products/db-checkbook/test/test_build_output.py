...
def test_group_checkbook():
    expected_result = (
        generate_expected_grouped_checkbook().set_index("fms_id").sort_index()
    )
    result = (
        _group_checkbook(_clean_checkbook(CHECKBOOK_TEST))
        .set_index("fms_id")
        .sort_index()
    )
...
@pytest.mark.skip(
    reason="TODO QA output of category assignment on budget code and contract purpose"
)
