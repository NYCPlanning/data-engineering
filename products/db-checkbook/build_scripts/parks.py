...
def read_csv_to_df(
    dir: Path = LIB_DIR, fn: str = "dpr_parksproperties.csv"
) -> pd.DataFrame:
    """read csv to df"""
...
def run() -> None:
    ...
    print(
        "% projects mapped to geoms before adding parks: {}".format(
            csdb[csdb["has_geometry"] == True].shape[0] / csdb.shape[0]
        )
    )
    ...
    print(
        "% projects mapped to geoms AFTER adding parks: {}".format(
            csdb_with_parks[csdb_with_parks["has_geometry"] == True].shape[0]
            / csdb_with_parks.shape[0]
        )
    )
    return
