...
borough_map = {"MN": "1", "BX": "2", "BK": "3", "QN": "4", "SI": "5"}
...
variances = {"NONE": {"token": ""}, "MOE": {"token": "_moe"}, "CV": {"token": "_cv"}}
...
measures = {
    ...
    "RATE": {"token": "_rate"},
    "INDEX": {"token": "_index"},
    "MEDIAN": {"token": "_median"},
    "PERCENTAGE_POINT": {"token": "_pnt"},
}
...
def download_df(geography, category, filename, appended_columns, copy_columns):
    ...
    if geography == "district":
        df[geography_column_names[geography]] = df[
            geography_column_names[geography]
        ].str.lstrip("0")
    try:
        df = df.set_index(geography_column_names[geography])
    except KeyError:
        raise KeyError(
            f"Unable to set index for file {filename}, category {category}, and geography {geography}"
        )

    df.columns = pd.MultiIndex.from_tuples(
        [(filename, col_name) for col_name in df.columns.tolist()],
        names=["file", "column"],
    )
    return df
...
def build_column_name(
    variable, subgroup, measure, variance, year="", subcategory="", suffix=""
):
...
def build_row_column_names(
    variable, subgroup, measures, variances, year, subcategory, suffixes
):
    result = []
    _suffixes = ["" for i in variances] if suffixes is None else suffixes
...
def get_headers(measures):
    if measures == ["COUNT", "COUNT", "COUNT", "PERCENT", "PERCENT"]:
        return [
            [["number", 3], ["percent", 2]],
            [["estimate", 1], ["moe", 1], ["cv", 1], ["estimate", 1], ["moe", 1]],
        ]
    elif measures == ["COUNT", "PERCENT"]:
...
def get_change_headers(measures):
    if measures == ["COUNT", "COUNT", "COUNT", "PERCENT", "PERCENT"]:
        ...
        return [
            [["number", 2], ["percent", 2]],
            [["estimate", 1], ["moe", 1], ["estimate", 1], ["moe", 1]],
        ]
    elif measures == ["MEDIAN", "MEDIAN", "MEDIAN"]:
        return [
            [["number", 2], ["percent", 2]],
...
def build_config(table_config, subgroup):
    ...
    if "denominator" in table_config:
        ...
        if "placeholder" in table_config["denominator"]:
            result["denominator_placeholder"] = table_config["denominator"][
                "placeholder"
            ]
    else:
        result["has_denominator"] = False

    if "has_change" in table_config and table_config["has_change"]:
        # print(merge(table_config["change"]["denominator"], table_config["denominator"])["label"])
        result["change"] = {
            "measures": [
                get_change_measures(measures) for measures in result["measures"]
            ],
            "variances": [
                get_change_variances(variances) for variances in result["variances"]
            ],
            "headers": get_change_headers(result["measures"][0]),
        }

    result["upper_limits"] = [
        [None for j in result["labels"]] for i in result["vintages"]
    ]
    result["lower_limits"] = [
        [None for j in result["labels"]] for i in result["vintages"]
    ]
    ...
    for i in range(len(table_config["vintages"])):
        ...
        is_change = (
            True
            if i == len(table_config["vintages"]) - 1 and result["has_change"]
            else False
        )
        ...
        for j, variable in enumerate(variables):
            ...
            if j == 0 and "denominator" in table_config:
                ...
                if (
                    "ignore_subgroup" in table_config["denominator"]
                    and table_config["denominator"]["ignore_subgroup"]
                ):
                    _subgroup = "tot"
            elif "subcategory" in table_config:
                _subcategory = table_config["subcategory"]
            if is_change:
                column_vintage.append(
                    build_row_column_names(
                        variable,
                        _subgroup,
                        result["change"]["measures"][j],
                        result["change"]["variances"][j],
                        year,
                        _subcategory,
                        suffixes,
                    )
...
def build_row(
    ...
    new_row = {"label": label, "isDenominator": is_denominator, "cells": []}
    for k in range(len(values)):
        ...
        if (
            new_cell["measure"] not in ["PERCENT", "PERCENTAGE_POINT"]
            and new_cell["variance"] != "CV"
    ...
    if is_survey:
        if is_change:
            ...
            for measure in measure_set:
                reliability_map[measure] = get_change_reliability(
                    *tuple(
                        [values[i] for i, var in enumerate(measures) if var == measure]
                    )
                )
...
def build_vintages(table_list, df_row):
    ...
    for table_config in table_list:
        ...
        for vintage_index, vintage_label in enumerate(table_config["vintages"]):
            ...
            is_change_vintage = (
                True
                if vintage_index == len(table_config["vintages"]) - 1
                and table_config["has_change"]
                else False
            )
            new_vintage["isChange"] = is_change_vintage
            new_vintage["label"] = vintage_label
            new_vintage["headers"] = (
                build_column_headers(table_config["change"]["headers"])
                if is_change_vintage
                else build_column_headers(table_config["headers"])
            )
            new_vintage["rows"] = []
...
def main():
    ...
    appended_columns = {
        ...
        "housing_production_borough": [
            ("units_hi_newconstruction_count", 0),
            ("units_hi_preservation_count", 0),
            ("total_pct", 100),
        ],
        "housing_production_citywide": [
            ("units_hi_newconstruction_count", 0),
            ("units_hi_preservation_count", 0),
            ("total_pct", 100),
        ],
    ...
    for year in ["_2000", "_0812", "_1923"]:
        for geography in ["_puma", "_borough", "_citywide"]:
            ...
            for subgroup in ["", "_anh", "_bnh", "_wnh", "_hsp"]:
                columns = columns + [
                    (f"age_median{subgroup}{var}", np.nan)
                    for var in ["_pct", "_pct_moe"]
                ]
    ...
    for subgroup in ["anh", "bnh", "wnh", "hsp", "onh"]:
        for measure in ["count", "pct", "pnt"]:
            demo_change_copy[f"pop_change_{subgroup}_{measure}"] = (
                f"pop_{subgroup}_change_{measure}"
            )
    ...
    resolved_tables = {
        "district": {"hsaq": {"tot": [], "anh": [], "bnh": [], "wnh": [], "hsp": []}},
        "borough": {"hsaq": {"tot": [], "anh": [], "bnh": [], "wnh": [], "hsp": []}},
        "citywide": {"hsaq": {"tot": [], "anh": [], "bnh": [], "wnh": [], "hsp": []}},
    }
        ...
        for category, category_config in geography_config.items():
            ...
            for subgroup, subgroup_config in category_config.items():
                ...
                for table_config in subgroup_config:
                    output[geography][category][subgroup].append(
        ...
        for geography, geography_config in pages.items():
            ...
            for category, category_config in geography_config.items():
                ...
                for subgroup, table_list in category_config.items():
                    for table_config in table_list:
                        if (
                            "files" in table_config
                            and "placeholder" not in table_config
                        ):
