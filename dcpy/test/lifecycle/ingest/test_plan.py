...
class TestReadDefinition:
    """
    Tests configure.read_definition_file_file
...
class TestResolveConfig:
    ...
    def test_simple(self):
        config = plan.resolve_config("simple", definition_dir=INGEST_DEF_DIR)
        assert len(config.datasets) == 1

    def test_one_to_many(self):
        config = plan.resolve_config("one_to_many", definition_dir=INGEST_DEF_DIR)
