...
class IndentedPrint(BaseModel, arbitrary_types_allowed=True):
    ...
    def to_list_structure(self):
        ...
        for field in self.model_fields:
            ...
            if isinstance(value, IndentedPrint):
                value_element = value.to_list_structure()
            elif not isinstance(value, DataFrame) and not value and value != 0:
                value_element = None
            else:
                value_element = value
    ...
    def indent_and_flatten(self, li: list, indent: str, offset: str):
        def apply_indent(el, level):
            i = offset + indent * level
            return i + str(el)

        def flatten(_li: list, level) -> list:
            if _li == []:
                return _li

            if isinstance(_li[0], list):
                first = flatten(_li[0], level + 1)
            elif isinstance(_li[0], set):
                first = flatten(sorted(list(_li[0])), level + 1)
            elif isinstance(_li[0], dict):
                first = []
                for key in _li[0]:
                    first.append(key)
                    first.append([_li[0][key]])
                first = flatten(first, level + 1)
            elif isinstance(_li[0], DataFrame):
                first = flatten(_li[0].to_string().split("\n"), level + 1)
            else:
                first = [apply_indent(_li[0], level)]
            return first + flatten(_li[1:], level)

        return flatten(li, level=0)

    def report(self):
        return "\n".join(
            self.indent_and_flatten(self.to_list_structure(), offset="", indent="    ")
        )
