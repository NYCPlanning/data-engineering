...
def get_config(name: str, version="latest") -> library.Config | ingest.DatasetConfig:
    """Retrieve a recipe config from s3."""
    config = get_config_obj(name, version)
    if "dataset" in config:
        return library.Config(**config)
    else:
        return ingest.DatasetConfig(**config)


def try_get_config(dataset: Dataset) -> library.Config | ingest.DatasetConfig | None:
