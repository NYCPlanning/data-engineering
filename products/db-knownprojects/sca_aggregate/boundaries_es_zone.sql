            ...
            left join
                ...
                on
                    case
                        ...
                        when
                            a.record_id in (select record_id from zap_project_many_bbls)
            ...
            select
                ...
                case
                    when concat(a.source, a.record_id) in (select concat(source, record_id) from multi_geocoded_projects) and st_area(a.geometry) > 0
                        then
                            (st_area(st_intersection(a.geometry, a.es_zone_geom)) / st_area(a.geometry))::decimal
                    else
                        1
        ...
        */
        ...
        aggregated_boundaries_es_zone_4 as (
            select
                a.*,
                case
                    when b.total_proportion is not null then (a.proportion_in_es_zone / b.total_proportion)::decimal
                    else 1
                end as proportion_in_es_zone_1,
                case
                    when b.total_proportion is not null then round(a.units_net * (a.proportion_in_es_zone / b.total_proportion)::decimal)
                    else a.units_net
                end as units_net_1
            from
