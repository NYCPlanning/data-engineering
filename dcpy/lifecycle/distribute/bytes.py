...
def _get_ids_to_paths_and_dests(product_metadata: md_v2.Metadata, destination_id: str):
...
def pull_destination_files(
    ...
    *,
    ...
    ids_to_paths_and_dests = _get_ids_to_paths_and_dests(
    ...
    for f in dest.files:
        ...
        if unpackage_zips and f.id in package_ids:
            ...
            assemble.unpackage(
                package_zip=file_path,
                out_path=local_package_path,
                package_id=f.id,
                product_metadata=product_metadata,
            )


def pull_all_destination_files(
    local_package_path: Path, product_metadata: md_v2.Metadata
):
...
def _product_from_bytes_cli(product_metadata_path: Path, product: str, version: str):
    for folder in [
        ...
        # assumeds the folders == the name of the dataset
        _dataset_from_bytes_cli(product_metadata_path, product, version, folder.name)
