...
- [Bug 017](./017-saf-abcegnpx-stale-prod-source-snapshot.md): `saf_abcegnpx_generic`/`saf_abcegnpx_roadbed`'s small row-count-only diffs (7 and 3 rows) trace to the same stale-prod-source-snapshot pattern as `CSCL-SAF-01`, `Bug 010`, and `Bug 011`, not the previously-suspected unimplemented SAF-replicant scope - 4 segments traced individually, all consistent with prod's derived SAF output not being fully regenerated each cycle
