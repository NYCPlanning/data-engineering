...
- [Bug 011](./011-altnames-saf-replicant-join-ids.md): `gdb_altnames`'s ~50%-of-prod `Join_ID` coverage gap was missing SAF-replicant Join_IDs entirely (CommonPlace/AddressPoint-sourced, per the legacy C# ETL's `GetSAFBytesJoinID`) - fixed for those two SAF types, closing ~85% of the gap (51% → 93% coverage); ALTSEGMENTDATA-sourced SAF Join_IDs deliberately not implemented (likely already covered by the regular path) - not yet verified via a full CI build
