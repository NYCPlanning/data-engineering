...
-- Compressed diff view showing one row per _universal_word_key with changes in jsonb
-- Includes status: modified, only_in_legacy, only_in_build

WITH base_diffs AS (
  {{ generate_diff_summary(
      old_relation=ref('qa_int__prod_universal_word'),
      new_relation=ref('universal_word'),
      primary_key='_universal_word_key',
      output_file_id='universal_word',
      build_table_name='universal_word',
      production_table_name='qa_int__prod_universal_word'
  ) }}
),
categorized AS (
    SELECT
...
SELECT
    _universal_word_key AS comparison_id,
    status,
    changes,
    output_file_id,
