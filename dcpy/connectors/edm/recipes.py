...
def archive_dataset(config: ingest.Config, file_path: Path, latest: bool = True):
    """
    ...
    if latest:
        s3.copy_folder(
            BUCKET,
            str(config.dataset_key.s3_path("ingest_datasets")) + "/",
            str(Path("ingest_datasets") / config.name / "latest") + "/",
            config.acl,
        )
