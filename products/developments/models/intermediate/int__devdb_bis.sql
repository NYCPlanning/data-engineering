WITH bis_job_applications AS (
    SELECT * FROM {{ ref('stg__jobs_bis') }}
),

lookup_ownership AS (
    SELECT * FROM {{ ref('lookup_ownership') }}
)

SELECT
    left(job_filing_number, strpos(job_filing_number, '-') - 1)::text AS job_number,
    CASE
        WHEN jobtype in ('ALT-CO - New Building with Existing Elements to Remain', 'Alteration CO') THEN 'Alteration'
        ELSE jobtype
    END AS job_type,
    CASE
        WHEN jobdescription !~ '[a-zA-Z]' THEN NULL
        ELSE jobdescription
    END AS job_desc,
    ...
    -- TODO: should this use calculated job types from above?
    (CASE
        WHEN jobtype ~* 'Alteration' THEN nullif(existing_stories, '0')
    END)::numeric AS stories_init,
    (CASE
        WHEN jobtype ~* 'Alteration' THEN nullif(proposed_no_of_stories, '0')
